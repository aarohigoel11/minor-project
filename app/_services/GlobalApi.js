...
const CreateNewStudent = async (data) => {
  try {
    const resp = await axiosInstance.post('/app/api/student/route.js', data); // Adjusted path
    return resp.data;
  } catch (error) {
...
const GetAttendanceList = async (batch, month) => {
  try {
    const resp = await axiosInstance.get(`/api/attendance/route?batch=${batch}&month=${month}`); // Adjusted path
    return resp.data;
  } catch (error) {
